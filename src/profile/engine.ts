...
const LANGUAGE_RULES: ProfileRule[] = [
  {
    condition: (a) => {
...
const TECH_STACK_RULES: ProfileRule[] = [
  {
...
const INTEREST_RULES: ProfileRule[] = [
  {
...
export function inferProfile(analysis: AnalysisResult): ProfileDraft {
  const inferences: ProfileInference[] = []
  ...
  for (const rule of LANGUAGE_RULES) {
    if (rule.condition(analysis)) {
      if (rule.result.language && profile.language === 'mixed') {
        profile.language = rule.result.language as 'zh' | 'en' | 'mixed'
        inferences.push({
          field: 'language',
...
export function suggestProtectedPaths(analysis: AnalysisResult): string[] {
...
export function suggestCatchAllTarget(analysis: AnalysisResult): string {
