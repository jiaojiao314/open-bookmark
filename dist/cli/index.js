...
import { inferProfile, saveProfile } from '../profile/engine.js';
...
program
    ...
    try {
        ...
        const bookmarks = JSON.parse(bookmarksData).map((b) => ({
            ...b,
...
program
    ...
    try {
        ...
        const { readFile } = await import('node:fs/promises');
        const bookmarksData = await readFile(state.bookmarks.snapshot, 'utf-8');
        const currentBookmarks = JSON.parse(bookmarksData).map((b) => ({
            ...b,
            dateAdded: new Date(b.dateAdded),
            dateModified: new Date(b.dateModified)
        }));
...
program
    ...
    try {
        ...
        await writeFile(state.bookmarks.snapshot, JSON.stringify(backupBookmarks, null, 2), 'utf-8');
        console.log('\n✅ 回滚完成!');
