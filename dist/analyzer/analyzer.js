...
/** Analyze bookmarks */
export function analyze(bookmarks) {
    // Domain counts
...
function sortedDomainInfos(counts, limit) {
    const infos = Array.from(counts.entries())
        .map(([domain, count]) => ({ domain, count }))
        .sort((a, b) => b.count - a.count);
    return limit > 0 ? infos.slice(0, limit) : infos;
}
/** Extract keywords from bookmark titles */
function extractKeywords(bookmarks) {
...
/** Detect URL patterns */
function detectPatterns(bookmarks, domainCounts) {
    let internalCount = 0;
    let githubCount = 0;
    let blogCount = 0;
    let aiCount = 0;
    ...
    for (const b of bookmarks) {
        const domain = extractDomain(b.url);
        if (isInternalURL(b.url))
            internalCount++;
        if (domain.includes('github.com') || domain.includes('gitee.com'))
            githubCount++;
        if (BLOG_DOMAINS.some(d => domain.includes(d)))
            blogCount++;
        if (AI_DOMAINS.some(d => domain.includes(d)))
            aiCount++;
        if (DEVOPS_DOMAINS.some(d => domain.includes(d)))
            devopsCount++;
    }
    // Blog domains breakdown
    const blogDomainCounts = new Map();
    for (const b of bookmarks) {
        const domain = extractDomain(b.url);
        if (BLOG_DOMAINS.some(d => domain.includes(d))) {
